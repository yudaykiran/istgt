...
dequeue_common_sendq:
		...
		TAILQ_FOREACH(replica, &spec->rq, r_next) {
			...
			if(cmd_read && !send_multiple_read && replica->state != REPLICA_HEALTHY) {
				continue;
			}
...
handle_read_resp(spec_t *spec, replica_t *replica)
{
	int luworker_id;
	bool io_found = false;
	rcmd_t *rep_cmd = NULL, *tmp_rep_cmd;
	...
	for (rep_cmd = TAILQ_FIRST(&replica->read_waitq); rep_cmd != NULL; rep_cmd = tmp_rep_cmd) {
		tmp_rep_cmd = TAILQ_NEXT(rep_cmd, rread_cmd_next);
